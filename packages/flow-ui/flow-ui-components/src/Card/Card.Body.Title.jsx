...
const CardBodyTitle = ({ variant, title, slug, link, price }) => {
  ...
  return (
    <div {...linkProps} sx={{ variant: rv(variant, 'title') }} style={{height: 68, fontSize: 16, marginRight: 8}}>
      {title} <button style={{borderWidth: 0, borderRadius: 2, padding: "4px", fontWeight: "bold",color: "#545454"}}>{price && price.toLocaleString('it-IT', {style : 'currency', currency : 'VND'})}</button>
    </div>
  )
}
